...
```bash
# Full v1 pipeline (fetch → process → digest)
python3 -m bookmark_digest run [--limit N]

# Generate v1 digest from pending items
python3 -m bookmark_digest digest [--limit N] [--category AI]

# Build profile from bird CLI bookmarks (v1)
python3 -m bookmark_digest profile [--limit 200]
```
...
Agent:
→ python3 -m bookmark_digest fetch --json --limit 30
  → Fetched 12 new bookmarks

→ python3 -m bookmark_digest build-llm-task-request --json < fetch_output.json
  → Built LLM prompt with schema + profile context

→ llm-task --schema schemas/bookmark-analysis-v1.json < prompt.json
  → LLM analyzed 12 bookmarks

→ python3 -m bookmark_digest store-analyses --json < llm_output.json
  → Stored 12 analyses

→ python3 -m bookmark_digest deliver --json --batch-size 5
  → Sent batch 1 (5 items) to Telegram with action buttons
...
Done. 12 bookmarks processed, 10 delivered, 2 remaining.
```
