...
def test_build_analysis_prompt_button_guide():
    ...
    assert "Button Selection Guide" in prompt
...
def test_format_delivery_message_buttons():
    ...
    item = {
        ...
        "buttons_json": json.dumps(["dd", "fc", "rs"]),
    }
    ...
    assert all_buttons[1]["callback_data"] == "q|fc|bk_abc"
    assert all_buttons[2]["callback_data"] == "q|rs|bk_abc"
...
def test_format_delivery_message_button_rows():
    ...
    item = {
        ...
        "category": "Tech",
        "analysis": json.dumps({"analysis": "test"}),
        "buttons_json": json.dumps(["dd", "im", "fc"]),
    }

    msg = format_delivery_message(item)
    assert len(msg["buttons"]) == 2  # [dd, im] and [fc]
...
def test_deliver_footer_format():
    """deliver output includes batch footer with next button."""
    with tempfile.TemporaryDirectory() as data_dir:
        db = os.path.join(data_dir, "queue.db")
        init_db(db)
        _make_analyzed_item(db, "t001")
        ...
        assert "items remaining" in footer["text"]
        assert len(footer["buttons"]) == 1
        assert footer["buttons"][0][0]["text"] == "Next 5 ▶"
