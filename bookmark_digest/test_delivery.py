...
def test_build_next_batch_button():
    ...
    rows = build_next_batch_button(batch_id, remaining=10, batch_size=5)
...
def test_build_next_batch_button_fewer_remaining():
    """Test next batch button shows actual remaining count when < batch_size."""
    rows = build_next_batch_button("batch1", remaining=3, batch_size=5)
    assert rows[0][0]["text"] == "Next 3 ▶"


def test_build_next_batch_button_none_remaining():
    """Test no button when nothing remaining."""
    rows = build_next_batch_button("batch1", remaining=0)
    assert rows == []


def test_build_next_batch_button_format():
    """Test next batch button callback format."""
    rows = build_next_batch_button("test_batch_123", remaining=5)
