...
def test_dry_run():
    """Test that dry-run doesn't mark items as triaged."""
    with tempfile.TemporaryDirectory() as data_dir:
        db = os.path.join(data_dir, "queue.db")
        init_db(db)
        add_item(db, sample_queue_item("test_dryrun"))
        
        pending_before = get_pending(db, limit=10)
        assert len(pending_before) == 1
        
        result = run_cli("--dry-run", "digest", data_dir=data_dir)
        assert result.returncode == 0
        
        pending_after = get_pending(db, limit=10)
        assert len(pending_after) == 1, "Dry-run should not triage items"
        assert pending_after[0].get("triaged_at") is None
        print("✓ CLI: digest --dry-run")
